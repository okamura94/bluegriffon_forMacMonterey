...
void MacIOSurfaceLib::LoadLibrary() {
  ...
  printf("--------MacIOSurface.cpp LoadLibrary() MacOS Version major: %d minor: %d\n", major, minor);
  printf("--------MacIOSurface.cpp LoadLibrary() OpenGL Library Path & Library Name: %s\n", kOpenGlLibPathName);
...
//  sOpenGLFramework = dlopen(OPENGL_FRAMEWORK_PATH,
  sOpenGLFramework = dlopen(kOpenGlLibPathName,
                            RTLD_LAZY | RTLD_LOCAL);
