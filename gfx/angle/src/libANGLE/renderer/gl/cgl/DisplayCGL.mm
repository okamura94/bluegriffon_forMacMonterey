...
egl::Error DisplayCGL::initialize(egl::Display *display)
{
    ...
    kOpenGlLibPathName = (char*)"/System/Library/Frameworks/OpenGL.framework/Libraries/libGL.dylib";
    if (major >= 11) {
       // >= Big Sur Version Lib Path & Lib Name Set
       kOpenGlLibPathName = (char*)"/System/Volumes/Data/usr/local/lib/libGLEW.dylib";
    }
    printf("--------DisplayCGL.mm initialize() MacOS Version major: %d minor: %d\n", major, minor);
    printf("--------DisplayCGL.mm initialize() OpenGL Library Path & Library Name: %s\n", kOpenGlLibPathName);
...
//    void *handle = dlopen(kDefaultOpenGLDylibName, RTLD_NOW);
    void *handle = dlopen(kOpenGlLibPathName, RTLD_NOW);
    if (!handle)
    {
