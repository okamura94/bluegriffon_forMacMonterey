...
nsXULTooltipListener::FindTooltip(nsIContent* aTarget, nsIContent** aTooltip)
{
  ...
  if (!tooltipText.IsEmpty()) {
    // tooltip Text Color (white->black) Change Start ------------------------------------
    // Get default tooltipId"default_tooltipId"
    nsCOMPtr<nsIContent> default_tooltipEl  = document->GetElementById(NS_LITERAL_STRING("default_tooltipId"));

    if (default_tooltipEl) {
#ifdef MOZ_XUL
      mNeedTitletip = false;
#endif
      // <label>-Label Get
      nsCOMPtr<nsIContent> default_tooltipEl_child = default_tooltipEl->GetFirstChild();
      
      // <label>-Label Value(tooltipText) Change
      default_tooltipEl_child->SetAttr(kNameSpaceID_None, nsGkAtoms::value, tooltipText, true);
      default_tooltipEl.forget(aTooltip);
      return NS_OK;
    }
    // tooltip Text Color (white->black) Change End --------------------------------------
