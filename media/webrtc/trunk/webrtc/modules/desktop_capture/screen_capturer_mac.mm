...
void ScreenCapturerMac::ScreenConfigurationChanged() {
  ...
  printf("--------screen_capturer_mac.mm ScreenConfigurationChanged() MacOS Version major: %d minor: %d\n", major, minor);
  printf("--------screen_capturer_mac.mm ScreenConfigurationChanged() OpenGL Library Path & Library Name: %s\n", kOpenGlLibPathName);
...
//  opengl_library_ = dlopen(kOpenGlLibraryName, RTLD_LAZY);
  opengl_library_ = dlopen(kOpenGlLibPathName, RTLD_LAZY);
  if (!opengl_library_) {
    // Change OpenGL Library Path & Library Name 2022.11
//    LOG_F(LS_ERROR) << "Failed to open " << kOpenGlLibraryName;
    LOG_F(LS_ERROR) << "Failed to open " << kOpenGlLibPathName;
    abort();
  }
